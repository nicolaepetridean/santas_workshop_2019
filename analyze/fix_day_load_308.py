...
if __name__ == '__main__' :
    ...
    second_best_item_switch = None
    second_best_item_day = None
    second_best_item_cost = 0
    second_freed_day = 0

    third_best_item_switch = None
    third_best_item_day = None
    third_best_item_cost = 0
    third_freed_day = 0
        ...
        for ch in range(1, 5):
            ...
            if best_item_switch is None:
                ...
                if ch_cost + int(acc_cost) < best_item_cost:
                    if second_best_item_day != None:
                        third_best_item_switch = second_best_item_switch
                        third_best_item_day = second_best_item_day
                        third_best_item_cost = second_best_item_cost
                        third_freed_day = second_freed_day

                    second_best_item_switch = best_item_switch
                    second_best_item_day = best_item_day
                    second_best_item_cost = best_item_cost
                    second_freed_day = freed_day
    ...
    daily_load[second_freed_day] -= FAMILY_SIZE[second_best_item_switch]
    prediction['assigned_day'][second_best_item_switch] = second_best_item_day

    daily_load[third_freed_day] -= FAMILY_SIZE[third_best_item_switch]
    prediction['assigned_day'][third_best_item_switch] = third_best_item_day

    third_switches = []
    second_switches = []
    switches = []
    for item in range(data.shape[0]):
        if data.iloc[item, 0] == freed_day or data.iloc[item, 0] == second_freed_day or data.iloc[item, 0] == third_freed_day:
            if prediction['assigned_day'][item] != freed_day and FAMILY_SIZE[item] < 6 and data.iloc[item, 0] == freed_day :
                switches.append(item)
            if prediction['assigned_day'][item] != second_freed_day and FAMILY_SIZE[item] < 6 and data.iloc[item, 0] == second_freed_day :
                second_switches.append(item)
            if prediction['assigned_day'][item] != third_freed_day and FAMILY_SIZE[item] < 6 and data.iloc[item, 0] == third_freed_day :
                third_switches.append(item)
    ...
    third_sum = 0
    if len(switches) > 0:
        while daily_load[freed_day] < 299 and start < len(second_switches):
    ...
    start = 0
    if len(second_switches) > 0:
        while daily_load[second_freed_day] < 299 and start < len(second_switches):
            if sum + FAMILY_SIZE[second_switches[start]] < FAMILY_SIZE[second_best_item_switch]:
                current_day = prediction['assigned_day'][second_switches[start]]
                if daily_load[current_day] - FAMILY_SIZE[second_switches[start]] >= 125:
                    daily_load[second_freed_day] += FAMILY_SIZE[second_switches[start]]
                    prediction['assigned_day'][second_switches[start]] = second_freed_day
                    sum += FAMILY_SIZE[second_switches[start]]
                    EXCLUDE.append(second_switches[start])
            start += 1

    # start = 0
    # if len(third_switches) > 0:
    #     while daily_load[third_freed_day] < 299 and start < len(third_switches):
    #         if sum + FAMILY_SIZE[third_switches[start]] < FAMILY_SIZE[third_best_item_switch]:
    #             current_day = prediction['assigned_day'][third_switches[start]]
    #             if daily_load[current_day] - FAMILY_SIZE[third_switches[start]] >= 125:
    #                 daily_load[third_freed_day] += FAMILY_SIZE[third_switches[start]]
    #                 prediction['assigned_day'][third_switches[start]] = third_freed_day
    #                 sum += FAMILY_SIZE[third_switches[start]]
    #                 EXCLUDE.append(third_switches[start])
    #         start += 1
