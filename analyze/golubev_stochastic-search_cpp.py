...
constexpr
array < uint8_t, 14 > DISTRIBUTION
{2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 3, 5}; // You
can
setup
how
many
...
you
need
for swaps and what best choice use for each family
// {2, 5}
it
's mean the first random family will brute force for choices 1-2 and the second random family will brute force for choices 1-5
...
array < uint8_t, 5000 > read_submission(string
filename) {
...
for (int j = 0; j < assigned_days.size();
++j) {
...
double
calc(const
array < uint16_t, 5000 > & indices, const
array < uint8_t, DISTRIBUTION.size() > & change) {
    double
accounting_penalty = 0.0;
auto
daily_occupancy = daily_occupancy_;
int
preference_cost = preference_cost_;
...
void
reindex(const
array < uint16_t, DISTRIBUTION.size() > & indices, const
array < uint8_t, DISTRIBUTION.size() > & change) {
...
double
calc(const
array < uint8_t, 5000 > & assigned_days, bool
print = false) {
    int
preference_cost = 0;
double
...
const
vector < array < uint8_t, DISTRIBUTION.size() >> changes = []()
{
...
void
stochastic_product_search(Index
index, ExitFunction
fn) { // 15
'360'
000
...
µs / it
double
best_local_score = calc(index.assigned_days);
thread_local
std::mt19937
...
for (const auto & change: changes) {
    auto
score = index.calc(indices, change);
...
main()
{
    init_data();
auto
assigned_day = read_submission("../input/another-pytorch-implementation/submission.csv");
...
return (++start <= 1000);};
auto
time_exit = [start = high_resolution_clock::now()]()
{
return duration_cast < minutes > (high_resolution_clock::now() - start).count() < 355; // 5
h55
};
